...
def generar_combinaciones(materias, rango, prefs, horas_libres):
    ...
    for mat_display in materias:
        mat_key = mat_display # Clave exacta
...
def create_pro_pdf(horario, alumno_data, total_creditos):
    ...
    for clase in horario_ordenado:
        materia_clean = clase['materia'] # Nombre directo
...
def create_timetable_html(horario):
    ...
    for clase in horario:
        ...
        if "Controladores Lógicos" in full_name: mat_name = "PLC"
            ...
            mat_name_display = full_name
            # Si tiene emoji y espacio, tratar de quitarlo SOLO para display visual corto
            if " " in full_name: 
                parts = full_name.split(' ', 1)
                if len(parts)>1: mat_name_display = parts[1]
            mat_name = mat_name_display.split(' ')[0] 
