...
def get_opiniones_data_cached(_client_status):
    ...
    try:
        ...
        for row in data:
            ...
            if prof not in opiniones_dict: opiniones_dict[prof] = {"suma": 0, "votos": 0, "comentarios": []}
...
def format_json_to_oferta(json_data):
    ...
    for mat_key, mat_info in json_data.get("materias", {}).items():
        ...
        if 1 <= semestre <= 9: materias_por_semestre[semestre].append(nombre)
        oferta_clases[nombre] = []
        for grupo in mat_info.get("grupos", []):
            formatted_horario = []
            for h in grupo.get("horario", []): formatted_horario.append((h["dia"], h["inicio"], h["fin"]))
...
def create_pro_pdf(horario, alumno_data, total_creditos, carrera_nombre):
    ...
    for clase in horario_ordenado:
        ...
        pdf.cell(w_prof, h_table, clean_text(clase['profesor'].split('(')[0]), 1)
        pdf.cell(w_salon, h_table, clean_text(clase.get('salon', 'TBA')), 1, 0, 'C')
        pdf.cell(w_cred, h_table, str(CREDITOS.get(clase['materia'], 0)), 1, 0, 'C')
        for d in range(6 if tiene_sabado else 5):
            txt_hora = ""
...
if menu == "📅 Generador de Horarios":
    if st.session_state.step == 1:
        ...
        bloqueo = num_selected != st.session_state.num_materias_deseadas
        for materia in selected_in_this_step:
            if materia in SERIACION:
                for req in SERIACION[materia]:
                    if req in selected_in_this_step: st.error(f"❌ {materia} requiere {req}."); bloqueo = True
        if bloqueo: st.warning("⚠️ Ajusta tu selección.")
        else:
        ...
        with col_rang: rango = st.slider("Horario Global:", 7, 22, (7, 22)); st.session_state.rango_hora = rango
        with col_free: libres = st.multiselect("Bloquear horas:", [f"{h}:00-{h+1}:00" for h in range(7, 22)]); st.session_state.horas_libres = libres
        col1, col2 = st.columns([1,1])
        ...
        for mat in st.session_state.materias_seleccionadas:
            with st.container(border=True):
                ...
                if not profes_validos: st.warning(f"⚠️ Sin profes para {mat}.")
                cols = st.columns(3)
                    ...
                    with cols[i % 3]:
                        ...
                        with st.expander("🕒 Horario y Reportes"):
                            ...
                            for sec in teacher_sections:
                                ...
                                with c_chk: st.checkbox(f"{sec['horario'][0][1]}:00-{sec['horario'][0][2]}:00", value=True, key=f"time_{mat}_{p}_{sec['horario'][0][1]}")
                                with c_warn:
                                ...
                                with c_btn:
                                    if st.button("📢 Lleno", key=f"rep_{spec_id}"):
                                        if add_group_report(db_client, spec_id): st.toast("Reporte enviado."); time.sleep(1); st.rerun()
        ...
        if st.session_state.resultados:
            ...
            for i, (score, horario) in enumerate(res[:10]): 
                with st.container(border=True):
                    ...
                    st.markdown(create_timetable_html(horario), unsafe_allow_html=True); st.write("")
        elif st.session_state.resultados is not None and len(st.session_state.resultados) == 0: st.warning("⚠️ Sin combinaciones.")
        if st.button("🔄 Inicio"):
    ...
    if not st.session_state.oferta_actual: st.warning("⚠️ Carga una carrera en la pestaña principal primero.")
    else:
        ...
        with c1:
            ...
            if st.button("Enviar Opinión"):
                if save_opinion(db_client, prof_selected, nuevo_comentario, nueva_calif): st.success("¡Registrada!"); time.sleep(1); st.rerun()
                else: st.error("Error.")
        with c2:
            if prof_selected in opiniones_reales:
                ...
                st.markdown(f"""<div style="display: flex; justify-content: center; margin-bottom: 10px;"><div style="width: 150px; height: 150px; border-radius: 50%; background: conic-gradient({color_chart} {promedio}%, #444 0); display: flex; justify-content: center; align-items: center;"><div style="width: 120px; height: 120px; border-radius: 50%; background: #1c1f26; display: flex; justify-content: center; align-items: center; color: white; font-weight: bold; font-size: 1.5em;">{promedio}/100</div></div></div><p style="text-align: center; color: #aaa;">Basado en {data['votos']} votos</p>""", unsafe_allow_html=True)
            else: st.info("Sin calificaciones.")
        st.write("---")
        st.subheader("💬 Comentarios Recientes")
        if prof_selected in opiniones_reales and opiniones_reales[prof_selected]["comentarios"]:
            for com in opiniones_reales[prof_selected]["comentarios"]: st.markdown(f"<div class='comment-bubble'>{com}</div>", unsafe_allow_html=True)
